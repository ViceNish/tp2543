						...
						<table class="table table-striped">
							...
							<tbody>
								...
								// for ($index = 0; $index < count($input_data); $index-=-1) {
									...
									$stmt = $conn->prepare("SELECT * FROM  tbl_products_a174088_pt2 WHERE FLD_PRODUCT_NAME LIKE :first_string AND FLD_PRICE LIKE :second_string AND FLD_TYPE LIKE :third_string");

									$stmt->bindParam(':first_string', $first_string, PDO::PARAM_STR);
									$stmt->bindParam(':second_string', $second_string, PDO::PARAM_STR);
									$stmt->bindParam(':third_string', $third_string, PDO::PARAM_STR);

									$first_string = $input_data[0];
									$first_string = "%$first_string%";
									$second_string = $input_data[1];
									$second_string = "%$second_string%";
									$third_string = $input_data[2];
